...
async def get(
    ...
    try:
        diagnoses_data = await controller.get_diagnoses_by_params(
            db, business_id, is_diagnosed
        )
...
async def post(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
    try:
        ...
        return JSONResponse(
            status_code=500, content={"message": "There was an error reading the file"}
        )
